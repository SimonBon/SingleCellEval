...
def mean_intensity(I, masks, channel_names=None):
    ...
    for n, val in enumerate(vals):
        ...
        if isinstance(channel_names, (list, tuple)):
            results[f"mean_intensity_{channel_names[n]}"] = val
            
        else:
            results[f"mean_intensity_channel_{n}"] = val
...
def mean_80_intensity(I, masks, channel_names=None):
    ...
    for n, val in enumerate(vals):
        ...
        if isinstance(channel_names, (list, tuple)):
            results[f"mean_80_intensity_{channel_names[n]}"] = val
            
        else:
            results[f"mean_80_intensity_channel_{n}"] = val
